...
Note that we always pass a plain ellipsis for every pattern, one could imagine
faster and/or more flexible ``__match__()`` implementations with more context,
but there are various downsides to this, see `rejected ideas`_.


Impossible matches
-------------------

Encouraged to try to revert the ``__init__()`` rather than represent internal state,
e.g. ``Foo([x, y, z])`` rather than ``Foo(x, y, z)`` if constructor always take a list.
*Strongly* encouraged to raise on impossible matches.
...
Standard library classes are not included here, can be added incrementally,
but some are important such as named tuples.
