...
Similarly, ``if not match`` is not allowed, since ``match`` is not an
expression.
...
illustrate this comparison well enough.
...
the dataclasses improvements, and for fot static typing. We propose to have
...
* It is hard to find an alternative keyword that would not be commonly used
  in existing programs as an identifier, and would stilll clearly reflect the
...
* It is a bit shorter so will save some keystrokes and horizontal space, which
  mat be important since this keyword will be repeated many times.
...
* This will significantly complicate the specification and implementation. In
  particular interaction with name pattern may be non-trivial.

* In real life, top-level alternative matches would be often split over
  multiple lines anyway. So this would look essentially not different from
...
* There was also on idea to make lookup semantics the default, and require
  ...
  But the name patterns are much more common in typical code, so having
  special syntax for common case would be weird.
...
* This can cause subtle bugs. In view this it is preferable that the safer
...
There was to allow multi-branch one-off matches of the following form::
...
Finally, this is easy to add later.
...
For example the below match would generate the following call::
  ...
  BinaryOp.__match__(
      (),
      {
          "left": PatternObject(Number, (), {"value": x}, -1, False),
          "op": ...,
          "right": PatternObject(Number, (), {"value": y}, -1, False),
      },
      -1,
      False,
  )

This would allow faster ``__match__()`` implementations and better support
customization in custom classes. There is however a big downside to this: it
will make basic implementation of this method really tedious. Also there will
be actual performance penalty if user does not treat pattern object properly.
...
* First of all, sets are unordered, so it is hard do define any deterministic
  semantics.

* Second, supporting ``frozenset`` is even more ambiguous and will be used very
  rarely. Namely, in addition to the general ordering issue, there are three
  possible forms to support ``frozenset([x, y, z])`` vs ``frozenset({x, y, z})``
  vs ``frozenset(x, y, z)``.

We can reconsider later if people will actually ask about supporting this.
